...
The output folder contains `duel-*.mp4` candidate clips and `duel-candidates.jsonl`, which records the estimated timestamp, type, confidence, and model note for every scanned window. The default threshold is 30 for broad coach review; use `--threshold 70` for fewer, stricter candidates or `--threshold 50` for a middle ground.
