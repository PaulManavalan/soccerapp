...
3. In **Project Settings → API Keys**, copy the Project URL and **Publishable key** (not the secret/service-role key).
4. Copy `supabase.config.example.js` to `supabase.config.js`, add the two values. Only the publishable key belongs in browser code.
5. In **Authentication → URL Configuration**, add `https://touchlinecoach.netlify.app` as the Site URL and an allowed redirect URL before using magic-link sign-in.
